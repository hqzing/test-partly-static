...
export CC_host="gcc"
export CXX_host="g++"
./configure --dest-cpu=arm64 --dest-os=openharmony --cross-compiling --prefix=node-${version}-openharmony-arm64-no-partly-static
make -j$(nproc)
make install

cp LICENSE node-${version}-openharmony-arm64-no-partly-static
tar -zcf node-${version}-openharmony-arm64-no-partly-static.tar.gz node-${version}-openharmony-arm64-no-partly-static
